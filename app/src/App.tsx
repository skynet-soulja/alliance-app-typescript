...
const reducer = (invoice: Invoice, action: Action) => {
  switch (action.type) {
    ...
    case "ADD-INVOICE-ITEM":
      const jobsiteId = (document.querySelector(
        "select[name=jobsite]"
      ) as HTMLInputElement).value;
      const ccId = (document.querySelector(
        "select[name=cc]"
      ) as HTMLInputElement).value;
      const modelId = (document.querySelector(
        "select[name=model]"
      ) as HTMLInputElement).value;
      const elevationId = (document.querySelector(
      ...
      const price = (() => {
        for (const key in priceMap) {
          if (
            key
              .split("|")
              .every((k, i) => k === priceIdCollection[i] || k === "*")
          ) {
            return priceMap[key];
          }
        }
      ...
      const newInvoiceItems = [
        ...invoice.invoiceItems,
        {
          ...
          price: price,
          priceFormatted: isNaN(parseInt(price))
            ? price
    ...
    case "REMOVE-INVOICE-ITEM":
      return {
        ...invoice,
        ...calculateTotal([
          ...invoice.invoiceItems.filter((_, i) => i !== action.index),
        ]),
      };
          ...
          <Form.Row>
            <Col id="btn-ctrl">
              <Button
                variant="primary"
                type="button"
                onClick={() => dispatch({ type: "ADD-INVOICE-ITEM" })}
              >
                Add Invoice Item
              </Button>
              <Button
                variant="primary"
                type="button"
                onClick={() => dispatch({ type: "RESET-INVOICE-ITEMS" })}
              >
                Clear Invoice
              </Button>
